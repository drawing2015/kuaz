package io.openmg.trike.graphdb.query;
...
import io.openmg.trike.core.QueryException;
 ...
 * However, when {@link io.openmg.trike.graphdb.query.ElementQuery#hasDuplicateResults()} is true (which assumes that the result set is sorted) then the merge sort iterator
...
public class QueryProcessor<Q extends ElementQuery<R, B>, R extends TitanElement, B extends BackendQuery<B>> implements Iterable<R> {
    ...
    private final class LimitAdjustingIterator extends io.openmg.trike.graphdb.query.LimitAdjustingIterator<R> {
