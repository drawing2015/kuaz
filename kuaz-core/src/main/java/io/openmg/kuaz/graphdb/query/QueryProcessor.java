package io.openmg.kuaz.graphdb.query;
...
import io.openmg.kuaz.core.QueryException;
 ...
 * However, when {@link io.openmg.kuaz.graphdb.query.ElementQuery#hasDuplicateResults()} is true (which assumes that the result set is sorted) then the merge sort iterator
...
public class QueryProcessor<Q extends ElementQuery<R, B>, R extends TitanElement, B extends BackendQuery<B>> implements Iterable<R> {
    ...
    private final class LimitAdjustingIterator extends io.openmg.kuaz.graphdb.query.LimitAdjustingIterator<R> {
